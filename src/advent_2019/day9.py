class IntcodeComputer:
    def __init__(self, memory, inputs):
        ...
        self.i = 0
        self.base_offset = 0
        [self.memory.append(0) for i in range(0, 5000)]
    ...
    def get_parameter(self, parameter_position):
        number = self.get_parameter_modes(self.opcode_string)[parameter_position - 1]
        if number == 0:
            return self.memory[self.i + parameter_position]
        elif number == 1:
            return self.i + parameter_position
        else:
            return self.memory[self.i + parameter_position] + self.base_offset
    ...
    def execute(self):
        ...
        while finished is False:
            ...
            if opcode == 1:
                ...
                self.base_offset += self.memory[self.get_parameter(1)]
                self.advance_position(2)
            elif opcode == 99:
                finished = True
